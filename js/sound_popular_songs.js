...
let songCounter = 0; // Contador de canciones reproducidas
const anuncios = [
    '../anuncios/Anuncio1.mp3',
    '../anuncios/Anuncio2.mp3',
    '../anuncios/Anuncio3.mp3',
    '../anuncios/Anuncio4.mp3'
]; // Lista de rutas de anuncios
...
// Función para cargar una canción o anuncio
function loadSong(index, songList = songs, isAd = false) {
    const song = isAd ? { src: getRandomAd(), title: 'Anuncio', cover: '' } : songList[index];
    if (!song) {
    ...
    sound = new Howl({
        ...
        onend: handleSongEnd, // Usar la nueva función handleSongEnd
    });
    ...
    currentSongImage.style.display = isAd ? "none" : "block";
...
function getRandomAd() {
    ...
    return anuncios[randomIndex];
}

// Función para manejar el final de una canción
function handleSongEnd() {
    if (songCounter === 1) {
        songCounter = -1; // Reiniciar el contador después de reproducir un anuncio
        loadSong(null, null, true); // Reproducir anuncio
    } else {
        songCounter++;
        console.log(`Canciones reproducidas: ${songCounter}`);
        playNextSong();
    }
}
